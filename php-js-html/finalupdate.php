			...
			<div class="container">
				...
				if(isset($_POST['proceed']))
				{
					...
					if(!empty($_POST['items'])) 
					{
						...
						foreach($_POST['items'] as $id) 
						{
							$r='pprice'.$id;
                            $price= $_POST[$r];
							$query= "UPDATE product SET price='$price' WHERE pid='$id'";
							$result=mysqli_query($db, $query);
                            //echo"updated item is".$id;
						}
				...
				if(mysqli_num_rows($result) > 0)
					...
					for($i=1; $row = mysqli_fetch_assoc($result); $i++)
						{
							echo "<tr><td> ".$i."</td><td> <img src='".$row['ipname']."' height=100px width=170px /></td><td> ". $row["productname"]. "</td><td>". $row["price"]. "</td></tr>" ;
						}
						echo "</table>";
					}
